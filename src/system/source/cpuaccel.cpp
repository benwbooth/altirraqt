...
#if defined(__x86_64__) || defined(__i386__)
    static inline void VDCPUID(int info[4], unsigned leaf) {
        __cpuid_count(leaf, 0, info[0], info[1], info[2], info[3]);
    }
    static inline void VDCPUIDEX(int info[4], unsigned leaf, unsigned subleaf) {
...
long CPUCheckForExtensions() {
	...
	if (cpuInfo[3] & (1 << 25)) {
		// Any x86_64 CPU supports SSE by definition; we drop the 32-bit Win32
		// SEH probe that the original Altirra code used.
		if (true) {
			...
			if ((cpuInfo[2] & ((1 << 27) | (1 << 28))) == ((1 << 27) | (1 << 28))) {
				// _xgetbv requires the xsave target feature even to issue the
				// intrinsic. We only reach here after verifying OSXSAVE is on,
				// so invoke via a target-attribute-tagged lambda.
				uint64 xcr0 = [] VD_CPU_TARGET("xsave") {
					return _xgetbv(0);
				}();
...
#elif VD_CPU_ARM64
long CPUCheckForExtensions() {
	// Feature detection on ARM64 POSIX is handled per-platform via getauxval()
	// or sysctlbyname(); we leave this as a stub until the port actually runs
	// on ARM64. Callers gracefully handle a zero-flags return.
	return 0;
}
#else
long CPUCheckForExtensions() {
