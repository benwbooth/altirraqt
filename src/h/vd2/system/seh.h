...
// Cross-platform Qt port: Win32's __try/__except (SEH) caught STATUS_ACCESS_
// VIOLATION, giving VDIsValidReadRegion / VDMemcpyGuarded a way to probe
// memory. POSIX has no portable equivalent, so we no-op the guard and
// trust call sites. Guarded probes become unconditional.
