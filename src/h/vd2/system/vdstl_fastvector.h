...
public:
	...
	~vdfastvector_base() {
		// Upstream asserts T is trivially copyable AND trivially destructible.
		// libstdc++'s std::pair has user-provided assignment operators that
		// trip the trivially-copyable check even when both members are POD.
		// Bit-copy is still safe for our concrete uses (pair<T*, bool>), so
		// we relax the assertion to trivially destructible only.
