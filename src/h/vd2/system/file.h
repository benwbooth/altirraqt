...
// Cross-platform Qt port: in the original code VDFileHandle mapped 1:1 to a
// Win32 HANDLE. On POSIX we use an ordinary file descriptor; the sentinel for
// "closed / not-open" is -1 rather than NULL.
typedef int VDFileHandle;
static constexpr VDFileHandle kVDFileHandleInvalid = -1;
...
class VDFile {
protected:
	VDFileHandle	mhFile = kVDFileHandleInvalid;
...
public:
	VDFile() : mhFile(kVDFileHandleInvalid) {}
