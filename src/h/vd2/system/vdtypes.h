...
#define VDASSERTCT(exp)	static_assert((exp), #exp)
	...
	// VDASSERT must compile away completely in release builds; the expression
	// is not evaluated. Upstream Altirra relies on this to reference members
	// (e.g. mCurrentPos) that only exist inside #ifdef _DEBUG blocks.
